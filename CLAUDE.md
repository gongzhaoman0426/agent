...
- **LLM**: LlamaIndex (OpenAI gpt-5.5 + OpenAI text-embedding-3-small)
- **Workflow**: 自定义 DSL 事件链执行器
- **Cache**: Redis (ioredis)
- **Auth**: better-auth
...
# Docker
docker compose up -d      # 启动基础设施 (PostgreSQL, Redis)
```
...
- `REDIS_URL` — Redis 连接串
- `OPENAI_MODEL` — OpenAI 对话模型，默认 `gpt-5.5`
